...
def start_quiz(n):
    ...
    # 20% fill-in-the-blank target
    n_fill = min(int(n * 0.2), len(fill_blank))
    n_multi = n - n_fill
    
    s_fill = fill_blank.sample(n=n_fill) if n_fill > 0 else pd.DataFrame()
    s_multi = multi.sample(n=n_multi) if n_multi > 0 else pd.DataFrame()
...
if not st.session_state.quiz_started:
    ...
    if idx < len(questions):
        ...
        with st.form("quiz_form"):
            if not is_fill:
                # Extract choices from text or use fallback
        ...
        if submitted:
            if not is_fill:
                ...
                # Support multiple correct answers in Column C separated by commas
    ...
    # --- CASE B: QUIZ FINISHED ---
    else:
        final_score = st.session_state.score
        total_qs = len(questions)
        percentage = (final_score / total_qs) if total_qs > 0 else 0
        
        st.write("## Quiz Complete")
        st.write(f"### Final Score: {final_score} / {total_qs} ({percentage:.0%})")
